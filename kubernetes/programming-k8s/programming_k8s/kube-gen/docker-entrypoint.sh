...
function kube::codegen::internal::grep_find() {
  pattern=$(kube::codegen::internal::generated_pattern '\w+')
  if [[ $pattern == "$2" ]]; then
    find "${GOPATH}/src/$1" -name "*.go" -print0 | xargs -0 grep -l "$2"
  else
    find "${GOPATH}/src/$1" -name "*.go" -print0 | xargs -0 grep -l -v "${pattern}" "$2"
  fi
}
...
function kube::codegen::internal::gv_inputs() {
  ...
  if kube::codegen::internal::not_empty "${gen_pkgs[@]}"; then
    ...
    echo "${gv_inputs[@]}" | xargs -n1 | sort -u
  fi
}
...
function kube::codegen::all() {
  ...
  kube::codegen::internal::remove_generated "$1" '\w+'
