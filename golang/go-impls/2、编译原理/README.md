...
中间代码是编译器或者虚拟机使用的语言，它可以帮助我们分析计算机程序。同时中间代码也是一种更洁净机器语言的表示形式，对中间代码的优化和分析相比直接分析高级编程语言更容易。

中间代码生成会在 `src/cmd/compile/internal/gc.Main` 方法中进行，首先会初始化 SSA 相关的配置，并最终调用 `compileFunctions` 来编译函数。

#### 配置初始化

在此过程中，我们会缓存可能用到的类型指针、初始化 SSA 配置和一些之后会调用的运行时函数。

#### 遍历和替换

在这个过程中是通过 `src/cmd/compile/internal/gc.walk` 等相关函数实现的。

#### SSA 生成

经过 Walk 系列函数ed处理之后，抽象语法树就不会改变了，Go 语言的编译器会使用 `src/cmd/compile/internal/gc.compileSSA` 函数将抽象语法树转换成中间代码。我们可以使用如下命令观察到整个过程：

```bash
GOSSAFUNC=funcname go build hello.go
```
...
Go 语言编译的最后一个阶段是根据 SSA 中间代码生成机器码，机器码的生成过程其实是对 SSA 代码的降级（lower）过程。

#### SSA 降级

SSA 降级是在中间代码生成过程中晚餐的。
